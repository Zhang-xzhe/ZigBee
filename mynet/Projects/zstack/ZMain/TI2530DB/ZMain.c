...
{
  // Turn off interrupts
  osal_int_disable( INTS_ALL );//就是EA=0
 
  // Initialization for board related stuff such as LEDs
  HAL_BOARD_INIT();//完成了板子的时钟切换、闪存，三个灯的输出，一个io口配置为输入三态

  // Make sure supply voltage is high enough to run
  zmain_vdd_check();//通过ADC检查电压状态

  // Initialize board I/O
  InitBoard( OB_COLD );//关IO口等，单片机行为

  // Initialze HAL drivers
  HalDriverInit();//设置外设

  // Initialize NV System
  osal_nv_init( NULL );//设置flash

  // Initialize the MAC
  ZMacInit();//关于协调器、路由器、终端的选择

...
#define PREFETCH_DISABLE()    st( FCTL = 0x04; )//0000 0100
...
{                                                                \
  ...
  asm("NOP");                               //一句asm(“nop”)延时了0.375us       chip bug workaround                            
  ...
  PREFETCH_ENABLE();                                             \//打开了一个寄存器，缓存使能，跟CPU的取指令有关
                                                                 \
  /* set direction for GPIO outputs  */                          \
  LED1_DDR |= LED1_BV;                                           \//将P1_0、P1_1、P1_4三个口配置为输出
  LED2_DDR |= LED2_BV;                                           \
  LED3_DDR |= LED3_BV;                                           \
                                                                 \
  /* configure tristates */                                      \
  P0INP |= PUSH2_BV;                                             \//将P0_0配置为三态
}
